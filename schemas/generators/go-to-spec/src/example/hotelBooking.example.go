...
func GenerateHotelBookings(guests *[]lodging.GuestProfile, nLines int, isSimpleBooking bool) []lodging.Booking {
	bookings := []lodging.Booking{}
	nEventsPerFile := nLines
	for i := 0; i < nEventsPerFile; i++ {
		bookings = append(bookings, CreateHotelBookingExample(guests, isSimpleBooking))
	}
	return bookings
}
...
func CreateHotelBookingExample(guestsPt *[]lodging.GuestProfile, isSimpleBooking bool) lodging.Booking {
...
func BuildHotelProduct() lodging.Product {
	rooms := map[string]lodging.RoomType{
		"SUITE": {Code: "SUITE", Name: "Suite", Description: "Luxury suite for a comfortable stays"},
		"DBL":   {Code: "DBL", Name: "Double room", Description: "Room with Double bed"},
		"KNG":   {Code: "KNG", Name: "King Room", Description: "Room with King Size Bed"},
	}
	ratePlans := map[string]lodging.RatePlan{
		"BAR":         {Code: "BAR", Name: "Best available rate", Description: "Our best available rate all year"},
		"SUPER_SAVER": {Code: "SUPER_SAVER", Name: "Super Saver rate", Description: "Lower price for advanced purchase"},
		"XMAS_WEEK":   {Code: "XMAS_WEEK", Name: "Xmas special rate", Description: "Special rate for the chrismas season"},
	}
	attributes := map[string]lodging.Attribute{
		"BALCONY":  {Code: "BALCONY", Name: "Balcony", Description: "Large balcony with chairs and a table"},
		"MINI_BAR": {Code: "MINI_BAR", Name: "Mini bar", Description: "Mini bar with local snacks and beverages"},
		"SEA_VIEW": {Code: "SEA_VIEW", Name: "Sea View", Description: "Full frontal view over the pacfic ocean"},
	}
	addOns := map[string]lodging.AddOn{
		"BKFT":    {Code: "BKFT", Name: "Suite", Description: "Our famous continental breakfast"},
		"TICKETS": {Code: "TICKETS", Name: "Suite", Description: "Tickets for our world reknown local theater"},
		"SPA":     {Code: "SPA", Name: "Suite", Description: "Luxury Spa session "},
	}
