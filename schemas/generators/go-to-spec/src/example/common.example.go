...
func Passenger(nLoyaltyTx int) air.PassengerProfile {
	...
	pax := air.PassengerProfile{
		...
		AlternateProfileIDs: []common.AlternateProfileID{},
	}
	...
	nAlternateProfileIds := gofakeit.Number(0, 2)
	for i := 0; i < nAlternateProfileIds; i++ {
		pax.AlternateProfileIDs = append(pax.AlternateProfileIDs, BuildAlternateProfileId())
	}
	return pax
}
...
func Guest(nLoyaltyTx int, simpleGuest bool) lodging.GuestProfile {
	...
	nAlternateProfileIds := gofakeit.Number(0, 2)
	for i := 0; i < nAlternateProfileIds; i++ {
		guest.AlternateProfileIDs = append(guest.AlternateProfileIDs, BuildAlternateProfileId())
	}
	return guest
}
...
func BuildPaxLoyalty(createdOn time.Time, nTx int) air.LoyaltyProgram {
	...
	return air.LoyaltyProgram{
		...
		Level:                    gofakeit.RandomString([]string{"bronze", "silver", "gold", "platinum"}),
		Joined:                   createdOn,
		Transactions:             txs,
		EnrollmentSource:         gofakeit.Word(),
		Currency:                 gofakeit.CurrencyShort(),
		Amount:                   core.Float(gofakeit.Price(1, 1000)),
		AccountStatus:            gofakeit.RandomString([]string{"active", "inactive", "suspended"}),
		ReasonForClose:           gofakeit.Sentence(5),
		LanguagePreference:       gofakeit.Language(),
		DisplayPreference:        gofakeit.RandomString([]string{"light", "dark"}),
		MealPreference:           gofakeit.RandomString([]string{"vegetarian", "vegan", "regular"}),
		SeatPreference:           gofakeit.RandomString([]string{"window", "aisle", "middle"}),
		HomeAirport:              AirportCode(),
		DateTimeFormatPreference: gofakeit.RandomString([]string{"12h", "24h"}),
		CabinPreference:          gofakeit.RandomString([]string{"economy", "business", "first"}),
		FareTypePreference:       gofakeit.RandomString([]string{"flexible", "standard", "basic"}),
		ExpertMode:               gofakeit.Bool(),
		PrivacyIndicator:         gofakeit.RandomString([]string{"private", "public"}),
		CarPreferenceVendor:      gofakeit.Company(),
		CarPreferenceType:        gofakeit.CarType(),
		SpecialAccommodation1:    gofakeit.Sentence(3),
		SpecialAccommodation2:    gofakeit.Sentence(3),
		SpecialAccommodation3:    gofakeit.Sentence(3),
		SpecialAccommodation4:    gofakeit.Sentence(3),
		SpecialAccommodation5:    gofakeit.Sentence(3),
		MarketingOptIns:          gofakeit.RandomString([]string{"email", "message", "call"}),
		RenewDate:                createdOn.AddDate(0, gofakeit.Number(1, 4), 0).String(),
		NextBillAmount:           core.Float(gofakeit.Price(50, 500)),
		ClearEnrollDate:          createdOn.AddDate(0, gofakeit.Number(1, 4), 0).String(),
		ClearRenewDate:           createdOn.AddDate(0, gofakeit.Number(4, 7), 0).String(),
		ClearTierLevel:           gofakeit.RandomString([]string{"basic", "plus", "premium"}),
		ClearNextBillAmount:      core.Float(gofakeit.Price(100, 1000)),
		ClearIsActive:            gofakeit.Bool(),
		ClearAutoRenew:           gofakeit.Bool(),
		ClearHasBiometrics:       gofakeit.Bool(),
		ClearHasPartnerPricing:   gofakeit.Bool(),
		TsaType:                  gofakeit.RandomString([]string{"pre-check", "global_entry"}),
		TsaSeqNum:                gofakeit.Regex("[A-Z0-9]{8}"),
		TsaNumber:                gofakeit.Regex("[0-9]{9}"),
	}
}
...
func BuildAlternateProfileId() common.AlternateProfileID {
	return common.AlternateProfileID{
		Name:          gofakeit.Noun(),
		Value:         gofakeit.UUID(),
		Description:   gofakeit.LoremIpsumSentence(5),
		LastUpdatedOn: gofakeit.Date(),
	}
}
