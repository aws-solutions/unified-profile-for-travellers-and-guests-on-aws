# This is my preferred approach, and the one I started with
# unfortunately it is very difficult to map on an array of objects then join
# from testing, so I have code that flattens automatically then applies a custom
# mapping and I use that, but I will keep this code here for reference

def buildObjectRecordOld(rec):
    try:
        arrayRec = []
        newRec = {}
...
def fillPaymentInformationDetails(startingString, dict, element):
    ...
    newRec = fillAddressDetails(
        "paymentInformation.address.", newRec, element["address"])
...
def generateAdressesArray(arrayRec, rec):
    ...
    for element in arr:
        newRecAddresses = {}
        newRecAddresses = fillAddressDetails(
            "holder.addresses.", newRecAddresses, element)
...
def generateProductsArray(arrayRec, rec):
    ...
    for element in arr:
        ...
        # holder
        newRecProducts = fillHolderDetails(
            "segments.products.holder", newRecProducts, element["holder"])
        # additionalGuests
        newRecProducts = fillHolderDetails(
            "segments.products.holder", newRecProducts, element["additionalGuests"])
