...
func TestTableCrud(t *testing.T) {
	...
	err := glueClient.CreateTable(tableName, bucket, map[string]string{"year": "int", "month": "int", "day": "int"})
	if err != nil {
		t.Errorf("[TestGlue] error creating table: %v", err)
	}
	table, err1 := glueClient.GetTable(tableName)
	if err1 != nil {
		t.Errorf("[TestGlue] error geting table: %v", err1)
	}
	...
	if len(errs) > 0 {
		t.Errorf("[TestGlue] error adding partitions to table: %v", errs)
	}
