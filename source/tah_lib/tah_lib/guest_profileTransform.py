...
BIZ_OBJECT_TYPE = "guest_profile"


class ObjectRecord(TypedDict):
    guest_profile_recs: list[dict[str, Any]]
    common_email_recs: list[dict[str, Any]]
    common_phone_recs: list[dict[str, Any]]
    hotel_loyalty_recs: list[dict[str, Any]]
    hotel_loyalty_tx_recs: list[dict[str, Any]]
...
def build_object_record(
        ...
        for loyalty in rec.get("loyaltyPrograms", []):
            ...
            for tx in loyalty.get("transactions", []):
                loyalty_tx_rec = {
                    ...
                    "qualifying_point_offset": parse_number(
                        tx.get("qualifyingPointOffset", "")
                    ),
    ...
    return {
        GUEST_PROFILE_RECORDS_KEY: profile_recs,
        COMMON_EMAIL_RECORDS_KEY: email_recs,
        COMMON_PHONE_RECORDS_KEY: phone_recs,
        HOTEL_LOYALTY_RECORDS_KEY: loyalty_recs,
        HOTEL_LOYALTY_TX_RECORDS_KEY: loyalty_tx_recs,
