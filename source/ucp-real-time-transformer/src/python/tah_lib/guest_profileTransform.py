...
from tah_lib.common import setPrimaryEmail, setPrimaryPhone, setPrimaryAddress, setTravellerId, setTimestamp, parseNumber, logErrorToSQS
...
def buildObjectRecord(rec, errQueueUrl):
    try:
        ...
        # format validation
        if profileRec['first_name'] == "" or profileRec['last_name'] == "":
            print("Invalid format Exception")
            raise Exception(
                "At least first OR last names are mandatory for ", BIZ_OBJECT_TYPE)
