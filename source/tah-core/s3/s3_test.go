...
func TestCreateDeleteBucket(t *testing.T) {
	...
	s3c := InitRegion(envCfg.Region, core.TEST_SOLUTION_ID, core.TEST_SOLUTION_VERSION)
...
func TestParseCSV(t *testing.T) {
	...
	s3c, err := InitWithRandBucket("tah-core-test-bucket-csv", "test_csv", envCfg.Region, core.TEST_SOLUTION_ID, core.TEST_SOLUTION_VERSION)
	if err != nil {
		t.Errorf("Could not initialize random bucket %+v", err)
	}
	log.Printf("Uploading CSV ../../test_data/ssi_core/csv1.csv")
	err = s3c.UploadFile("csv1.csv", "../../test_data/ssi_core/csv1.csv")
	if err != nil {
...
func TestSaveMany(t *testing.T) {
	...
	s3c, err := InitWithRandBucket("tah-core-test-bucket-csv", "test_mass_upload", envCfg.Region, core.TEST_SOLUTION_ID, core.TEST_SOLUTION_VERSION)
	if err != nil {
		t.Errorf("Could not initialize random bucket %+v", err)
	}
...
func TestS3MultiPart(t *testing.T) {
	...
	PATH := "test_multipart"
	s3Config, err0 := InitWithRandBucket("tah-core-test-bucket-csv", PATH, envCfg.Region, core.TEST_SOLUTION_ID, core.TEST_SOLUTION_VERSION)
	if err0 != nil {
	...
	if err != nil {
		t.Errorf("[StartMpUpload] Failed to start multipart Upload with error: %+v", err)
	}
...
func TestPresignUrl(t *testing.T) {
	...
	s3c, err := InitWithRandBucket("tah-core-test-bucket-csv", "test_presign", envCfg.Region, core.TEST_SOLUTION_ID, core.TEST_SOLUTION_VERSION)
	if err != nil {
		t.Errorf("Could not initialize random bucket %+v", err)
	}
	key := "mpUploadTest"
