...
type IS3Config interface {
	...
	Delete(path string, id string) error
	GetBucket() string
	GetRegion() string
}
...
func InitRegion(region, solutionId, solutionVersion string) S3Config {
...
func InitWithRandBucket(prefix, path, region, solutionId, solutionVersion string) (S3Config, error) {
	s3c := InitRegion(region, solutionId, solutionVersion)
	name, err := s3c.CreateRandomBucket(prefix)
	return Init(name, path, region, solutionId, solutionVersion), err
}

func Init(bucket, path, region, solutionId, solutionVersion string) S3Config {
...
func NewHTTPClientWithSettings(httpSettings HTTPClientSettings) *http.Client {
	...
	return core.CreateCustomClient(httpSettings.SolutionID, httpSettings.SolutionVersion, tr)
}
...
func (s3c *S3Config) CreateBucket(name string) error {
	s3c.Tx.Info("Creating Bucket %s", name)
...
func (s3c S3Config) CreateRandomBucket(prefix string) (string, error) {
	name := prefix + "-" + strings.ToLower(core.GenerateUniqueId())
	err := s3c.CreateBucket(name)
	return name, err
}

func (s3c S3Config) DeleteBucket(name string) error {
	s3c.Tx.Info("Deleting Bucket %s", s3c.Bucket)
...
func (s3c S3Config) EmptyBucket() error {
	s3c.Tx.Info("Emptying Bucket %s", s3c.Bucket)
	...
	if err := s3manager.NewBatchDeleteWithClient(s3c.Svc).Delete(aws.BackgroundContext(), iter); err != nil {
		s3c.Tx.Error("Unable to delete objects from bucket %q, %v", s3c.Bucket, err)
		return err
	}
	return nil
}

func (s3c S3Config) EmptySpecificBucket(bucketname string) error {
	s3c.Tx.Info("Emptying Bucket %s", s3c.Bucket)
	// Setup BatchDeleteIterator to iterate through a list of objects.
	iter := s3manager.NewDeleteListIterator(s3c.Svc, &s3.ListObjectsInput{
		Bucket: &bucketname,
	...
	// Traverse iterator deleting each object
	if err := s3manager.NewBatchDeleteWithClient(s3c.Svc).Delete(aws.BackgroundContext(), iter); err != nil {
		s3c.Tx.Error("Unable to delete objects from bucket %q, %v", bucketname, err)
		return err
	}
	return nil
}
...
func (s3c S3Config) EmptyAndDelete() error {
	s3c.Tx.Info("Emptying and deleting bucket %s", s3c.Bucket)
	err := s3c.EmptyBucket()
	if err != nil {
		s3c.Tx.Error("Could not empty")
		return err
	}
	err = s3c.DeleteBucket(s3c.Bucket)
	if err != nil {
		s3c.Tx.Error("Could not delete")
		return err
	}
	return nil
}
...
func (s3c S3Config) AddPolicy(name string, resources []string, actions []string, principals map[string][]string) error {
	s3c.Tx.Info("Adding Bucket policy %s", s3c.Bucket)
...
func (s3c S3Config) buildBucketPolicy(resources []string, actions []string, principals map[string][]string) string {
	...
	s3c.Tx.Info("Policy Created:  %s", string(policyJson))
...
func (s3c S3Config) SaveMany(paths []string, data []string, contentType string, batchSize int) []error {
	if len(paths) != len(data) {
		s3c.Tx.Error("[SaveMany] Length of S3 paths and data does not match (%v versus %v)\n", len(paths), len(data))
	...
	s3c.Tx.Debug("[SaveMany] splitting the content in \n", len(data))
	mu := &sync.Mutex{}
...
func (s3c S3Config) DeleteMany(paths []string, batchSize int) []error {
	...
	var errs []error
	batchedPaths := core.Chunk(core.InterfaceSlice(paths), batchSize)
	s3c.Tx.Debug("[DeleteMany] splitting the content in \n", len(paths))
	mu := &sync.Mutex{}
...
func (s3c S3Config) Save(path string, id string, data []byte, contentType string) error {
	...
	s3c.Tx.Info("Uploading data to S3 %+v", s3c.Bucket+"/"+key)
	result, err := s3c.Svc.PutObject(params)
	if err == nil {
		s3c.Tx.Debug("Result of upload to S3: %+v", result)
	} else {
		s3c.Tx.Error("Error while uploading to S3: %+v", err)
	}
	return err
}
...
func (s3c S3Config) CreatePresignedUrl(key string, dur time.Duration) (string, error) {
	...
	if err != nil {
		s3c.Tx.Error("[S3] Error for CreatePresignedUrl %+v", err)
		return "", err
	}
	return urlStr, err
}
...
func (s3c S3Config) Get(key string, object interface{}) error {
	...
	s3c.Tx.Debug("DOWNLOADING FROM S3 bucket %s at key %s", s3c.Bucket, s3c.Path+"/"+key)
...
func (s3c S3Config) Delete(path string, id string) error {
	...
	s3c.Tx.Info("[S3] Deleting object %+v", key)
	...
	if err != nil {
		s3c.Tx.Error("[S3] ERROR while deletin object %+v", err)
	}
	return err
}
...
func (s3c *S3Config) GetParquetObj(key string) (data string, err error) {
	fr, err := s3p.NewS3FileReader(context.Background(), s3c.Bucket, key, &s3c.Svc.Config)
	if err != nil {
		log.Printf("[S3] error reading file in bucket: %v", err)
		return "", err
	}
	defer fr.Close()
	...
	if err != nil {
		log.Printf("[S3] error creating parquet reader: %v", err)
		return "", err
	}
	defer pr.ReadStop()
	...
	if err != nil {
		log.Printf("[S3] error reading parquet: %v", err)
		return "", err
	}
	jsonBytes, err := json.Marshal(res)
	if err != nil {
		log.Printf("[S3] error marshalling json: %v", err)
		return "", err
	}
...
func (s3c S3Config) ParseCsvFromS3(fileName string) ([][]string, error) {
	...
	if err != nil {
		s3c.Tx.Error("Could not fetch CSV file form S3: %v", err)
		return [][]string{}, err
	}
...
func (s3c S3Config) Search(prefix string, maxRes int) ([]string, error) {
	s3c.Tx.Debug("[S3] listing object with prefix %+v", prefix)
	sess, _ := session.NewSession(&aws.Config{
		Region: aws.String(s3c.Region)},
	)
	svc := s3.New(sess)
	...
	if err != nil {
		s3c.Tx.Error("[S3] ERROR while listing object %+v", err)
	}
	res := []string{}
	...
	for result.IsTruncated != nil && *result.IsTruncated {
		s3c.Tx.Debug("[S3] List is truncated getting next page")
...
func (s3c S3Config) WaitForData(timeout int) error {
	log.Printf("Waiting for data in S3")
	start := time.Now()
	for {
		...
		log.Printf("Still no data. Waiting 5 sec")
		time.Sleep(5 * time.Second)
	}
}

func (s3c S3Config) WaitForObject(prefix string, timeout int) error {
	log.Printf("Waiting for object %v to be in S3", prefix)
	start := time.Now()
	for {
		res, err := s3c.Search(prefix, 100)
		if err != nil {
			return err
		}
		log.Printf("Objects in S3 buckets (truncated to first 100): %+v", res)
		for _, r := range res {
			if strings.HasPrefix(r, prefix) {
				return nil
			}
			if strings.HasPrefix(r, "error") {
				log.Printf("Ingestion erorr file has been added to S3: %v", r)
				data, _ := s3c.GetTextObj(r)
				log.Printf("Error file: %v", data)
				return fmt.Errorf("ingestion erorr file has been added to S3: %v. %v", r, data)
			}
		}
		if time.Since(start).Seconds() > float64(timeout) {
			return nil
...
func (s3c S3Config) UploadFileWithDetails(key string, filePath string) (ObjectDetails, error) {
	sess, _ := session.NewSession(&aws.Config{
		Region: aws.String(s3c.Region)},
	)
	// Create an uploader with the session and custom options
	...
	// Upload the file to S3.
	output, err2 := uploader.Upload(&s3manager.UploadInput{
		Bucket: aws.String(s3c.Bucket),
		Key:    aws.String(s3c.Path + "/" + key),
		Body:   f,
	})
	...
	if err2 != nil {
		log.Printf("File upload faile with error %+v", err)
		return ObjectDetails{}, err2
	}

	objectDetails := ObjectDetails{
		ETag:      strings.Replace(aws.StringValue(output.ETag), "\"", "", -1),
